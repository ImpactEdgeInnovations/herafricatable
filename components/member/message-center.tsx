...
export function MessageCenter({
  ...
  }, [router, selectedConversation, supabase]);
  async function send(e: FormEvent) {
    ...
    setBusy(false);
    setMessage(error ? memberErrorMessage(error, "send your message") : "");
    if (!error) {
      setBody("");
      router.refresh();
    }
  ...
  async function remove(id: string) {
    const result = await ask({
      ...
      description:
        "The message will be removed from the conversation for its participants.",
    ...
    setMessage(
      error
        ? memberErrorMessage(error, "remove this message")
        : "Message removed.",
    );
  ...
  async function report(id: string) {
    const result = await ask({
      ...
      fields: [
        ...
        {
          name: "block",
          label: "Also block this sender immediately",
          type: "checkbox",
        },
      ],
    });
    ...
    const block = Boolean(result.block);
    const { error } = await supabase.rpc("report_message", {
      p_block_sender: block,
    ...
    setMessage(
      error
        ? memberErrorMessage(error, "send this private message report")
        : "Message reported privately to moderators.",
    );
    if (!error) router.refresh();
  }
      ...
      <aside className="conversation-list">
        ...
        {conversations.length ? (
          conversations.map((item) => (
            <Link
              className={
                item.conversation_id === selectedConversation ? "selected" : ""
              }
            ...
            >
              ...
              <span>
                ...
                <small>{item.last_message || "Start the conversation"}</small>
              </span>
          ...
          <>
            ...
            <div className="message-thread">
              {[...messages].reverse().map((item) => (
                <article
                  className={item.sender_id === currentUserId ? "own" : ""}
                  key={item.message_id}
                >
                  <div>
                    <p>{item.body}</p>
                  ...
                  {item.status !== "deleted" ? (
                    <span>
                      {item.sender_id === currentUserId ? (
                        <button onClick={() => void remove(item.message_id)}>
                          Remove
                        </button>
                      ) : (
                        <button onClick={() => void report(item.message_id)}>
                          Report
                        </button>
                      )}
                    </span>
            ...
            <form className="message-composer" onSubmit={send}>
              <label>
                ...
                <textarea
                  ...
                  placeholder={`Message ${selected.display_name ?? "your connection"}…`}
                />
              </label>
              <button
