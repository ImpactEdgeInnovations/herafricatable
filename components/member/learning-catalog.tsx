...
export type CourseSummary = {
  ...
  access_type: "free" | "purchase" | "event_bundle" | "manual";
  price_minor: number;
  currency: string;
...
export function LearningCatalog({ courses }: { courses: CourseSummary[] }) {
  ...
  async function access(course: CourseSummary) {
    setBusy(course.course_id);
    if (
      course.access_type === "purchase" &&
      course.payment_mode === "automatic"
    ) {
      ...
      setBusy("");
      setMessage(
        memberErrorMessage(payload.error, "start checkout for this course"),
      );
      return;
    }
    if (
      course.access_type === "purchase" &&
      course.payment_mode === "manual_review"
    ) {
      ...
      const { error } = await supabase.rpc("create_course_order", {
        ...
        {courses.map((course) => (
          <article key={course.course_id}>
            <div className="learning-card-meta">
              <span>{course.access_type.replace("_", " ")}</span>
              <small>
                {course.lesson_count} lesson
                {Number(course.lesson_count) === 1 ? "" : "s"}
              </small>
            </div>
            <h2>{course.title}</h2>
            <p>{course.summary}</p>
            <div className="learning-instructor">
            ...
            <footer>
              {course.enrollment_status ? (
                ...
                course.payment_mode === "closed" ? (
                <span>Enrollment opening soon</span>
              ) : (
                <button
                  className="button button-outline"
                  disabled={busy === course.course_id}
                  onClick={() => void access(course)}
                >
                  {course.access_type === "purchase"
                    ? `Enroll · ${money(course.price_minor, course.currency)}`
                    : course.access_type === "event_bundle"
                      ? "Verify event access"
                      : "Enroll free"}
                </button>
              )}
