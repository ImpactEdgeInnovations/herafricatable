...
export function OpportunityMarketplace({
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.rpc("save_marketplace_post", {
      p_body: form.get("body"),
      p_category: form.get("category"),
      p_closes_at: closes ? new Date(`${closes}T23:59:00`).toISOString() : null,
      p_delivery_mode: form.get("delivery_mode"),
      p_industry: form.get("industry"),
      p_location: form.get("location"),
      p_post_id: editing?.post_id ?? null,
      p_post_type: form.get("post_type"),
    ...
    setMessage(
      error
        ? memberErrorMessage(
            error,
            editing ? "update your post" : "publish your post",
          )
        : editing
          ? "Post updated."
          : "Your post is live.",
  ...
  async function respond(post: MarketplacePost) {
    const result = await ask({
      ...
      description: `Write a thoughtful private response to “${post.title}”.`,
    ...
    const { error } = await supabase.rpc("respond_to_marketplace_post", {
      p_message: body,
  ...
  async function status(
    ...
    next: "archived" | "closed" | "published",
  ) {
    setBusy(post.post_id);
    const { error } = await supabase.rpc("set_marketplace_post_status", {
      p_post_id: post.post_id,
      p_status: next,
    });
    setBusy("");
  ...
  async function review(
    ...
    const { error } = await supabase.rpc("review_marketplace_response", {
    ...
    setMessage(
      error
        ? memberErrorMessage(error, `${next} this response`)
        : `Response ${next}.`,
    );
    if (!error)
  ...
  async function report(post: MarketplacePost) {
    const result = await ask({
    ...
    setBusy(post.post_id);
    const { error } = await supabase.rpc("report_marketplace_post", {
      p_category: category,
      p_details: details,
      p_post_id: post.post_id,
    });
        ...
        >
          <div className="opportunity-form-grid">
            <label>
              ...
              >
                <option value="ask">Ask — I need something</option>
            ...
            <label className="wide">
              ...
              <input
                ...
                placeholder="A precise, useful headline"
              />
            </label>
            <label className="wide">
              Details
              <textarea
                ...
                placeholder="What do you need or offer, who is it relevant to, and what would a good response include?"
              />
            </label>
            <label>
              Industry
            ...
            <label>
              ...
              >
                ...
                <option value="hybrid">Online or in person</option>
              </select>
            </label>
            <label>
              Close responses on
              <input
                name="closes_at"
                type="date"
                defaultValue={editing?.closes_at?.slice(0, 10) ?? ""}
              />
            </label>
          </div>
      ...
      <section className="opportunity-feed">
        <header>
          <div>
            ...
            <h2>Current asks and offers</h2>
          </div>
          <form>
            <input name="q" placeholder="Search needs, skills or industries" />
            <select name="type" defaultValue="">
              <option value="">Asks &amp; offers</option>
          ...
          <div className="opportunity-grid">
            {posts.map((post) => {
              ...
              const postResponses = responses.filter(
                (response) => response.post_id === post.post_id,
              );
              return (
                <article
                  className={`opportunity-card type-${post.post_type}`}
                  key={post.post_id}
                >
                    ...
                    {post.closes_at ? (
                      <span>
                        ...
                        {new Intl.DateTimeFormat("en-KE", {
                          dateStyle: "medium",
                        }).format(new Date(post.closes_at))}
                      </span>
                    ) : null}
                  </div>
                  <footer>
                    <div>
                      <strong>{mine ? "Your post" : post.author_name}</strong>
                      <small>
                        {[
                          post.author_role,
                          post.author_company,
                    ...
                    <span>
                      {post.response_count} response
                      {Number(post.response_count) === 1 ? "" : "s"}
                    </span>
                  </footer>
                  <div className="opportunity-actions">
                    {mine ? (
                      <>
                        ...
                        {post.status === "published" ? (
                          <button
                            ...
                            onClick={() => void status(post, "closed")}
                          >
                            Close
                          </button>
                        ) : (
                          <button
                            disabled={busy === post.post_id}
                            onClick={() => void status(post, "published")}
                          >
                            Reopen
                          </button>
                        )}
                        <button
                          disabled={busy === post.post_id}
                          onClick={() => void status(post, "archived")}
                        >
                          Archive
                        </button>
                      </>
                      ...
                      <>
                        {post.status === "published" &&
                        !post.own_response_status ? (
                          <button
                            className="primary"
                            disabled={busy === post.post_id}
                            onClick={() => void respond(post)}
                          >
                            Respond privately
                          </button>
                        ) : (
                  ...
                  {mine && postResponses.length ? (
                    <div className="opportunity-responses">
                      <strong>Private responses</strong>
                      {postResponses.map((response) => (
                        <section key={response.response_id}>
                          <div>
                            <b>{response.responder_name}</b>
                            <small>
                              {[
                          ...
                          <div>
                            ...
                            {response.status === "sent" ? (
                              <>
                                <button
                                  disabled={busy === response.response_id}
                                  onClick={() =>
                                    void review(response, "accepted")
                                  }
                                >
                                  Accept
                                </button>
