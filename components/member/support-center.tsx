...
export function SupportCenter({
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`support:${selectedTicketId}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  async function createTicket(event: FormEvent) {
    ...
    setBusy(false);
    if (error) {
      setNotice(memberErrorMessage(error, "create your support request"));
      return;
    }
    setSubject("");
    setDescription("");
    router.push(`/support?ticket=${data}`);
    router.refresh();
  }
  async function sendReply(event: FormEvent) {
    ...
    setBusy(false);
    if (error) {
      setNotice(memberErrorMessage(error, "send your support reply"));
      return;
    }
    setReply("");
    router.refresh();
          ...
          <form onSubmit={createTicket}>
            ...
            <label>
              ...
              <input
                ...
                placeholder="For example: Payment awaiting review"
                aria-describedby="support-subject-help"
              />
              <small className="form-help" id="support-subject-help">
                Use a short title that makes the request easy to recognise
                later.
              </small>
            </label>
            <label>
              ...
              <textarea
                ...
                placeholder="Tell us what you tried, what happened and what you need help with."
                aria-describedby="support-description-help"
              />
              <small className="form-help" id="support-description-help">
                Do not include passwords, one-time codes or full card details.
              </small>
            </label>
          ...
          <>
            ...
            <div className="support-thread">
              <article className="support-opening">
                <div>
                  <strong>You opened this request</strong>
              ...
              {messages.map((message) => (
                <article
                  className={
                    message.author_id === currentUserId ? "own" : "staff"
                  }
                  key={message.id}
                >
                  <div>
                    <strong>
                      {message.is_staff ? "Her Africa Table support" : "You"}
                    </strong>
                    <p>{message.body}</p>
            ...
            {selected.status !== "closed" ? (
              <form className="support-composer" onSubmit={sendReply}>
                <label>
                  ...
                  <textarea
                    ...
                    placeholder="Add a private reply…"
                  />
                </label>
                <button
              ...
              <p className="support-closed">
                This request is closed. Open a new request if you need more
                help.
              </p>
            )}
          </>
