...
const blankAlbum = {
  ...
  title: "Event moments",
  introduction: "",
  status: "draft" as AdminGalleryAlbum["status"],
  sortOrder: "0",
};
const blankAsset = {
  id: null as string | null,
  ...
  credit: "",
  capturedAt: "",
  status: "draft" as AdminMediaAsset["status"],
  isFeatured: false,
  sortOrder: "0",
};
...
function assetForm(asset: AdminMediaAsset) {
  return {
    ...
    credit: asset.credit ?? "",
    capturedAt: asset.captured_at ? asset.captured_at.slice(0, 16) : "",
    status: asset.status,
...
export function EventGalleryManager({
  ...
  events: AdminEvent[];
  initialAlbums: AdminGalleryAlbum[];
  ...
  const [album, setAlbum] = useState(() =>
    activeAlbum
      ? {
          ...
          introduction: activeAlbum.introduction ?? "",
          status: activeAlbum.status,
          sortOrder: String(activeAlbum.sort_order),
        }
      : blankAlbum,
  );
    ...
    setAlbum(
      nextAlbum
        ? {
            ...
            introduction: nextAlbum.introduction ?? "",
            status: nextAlbum.status,
            sortOrder: String(nextAlbum.sort_order),
          }
        : blankAlbum,
    );
  }
  ...
  function chooseAlbum(nextId: string) {
    ...
    setAlbum({
      ...
      status: next.status,
      sortOrder: String(next.sort_order),
    });
    setAsset(blankAsset);
    setFile(null);
    setMessage("");
  ...
  function showError(error: { message: string } | null) {
    ...
    setBusy(false);
    setMessage(adminErrorMessage(error, "save this event gallery change"));
    return true;
  }
  ...
  async function saveAlbum(event: FormEvent) {
    ...
    setBusy(true);
    setMessage("");
    const { error } = await supabase.rpc("save_gallery_album", {
      p_album_id: album.id,
      p_event_id: eventId,
      p_introduction: album.introduction,
      p_sort_order: Number(album.sortOrder) || 0,
      p_status: album.status,
      p_title: album.title,
    });
  ...
  async function chooseFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (chosen.size > 10 * 1024 * 1024) {
      setMessage("Event images must be 10 MB or smaller.");
      return;
    }
    ...
    setAsset({
      ...
      width,
      height,
      altText: chosen.name.replace(/\.[^.]+$/, "").replace(/[-_]/g, " "),
    });
    setMessage("");
  }
  ...
  async function saveAsset(event: FormEvent) {
    event.preventDefault();
    if (!albumId) {
      setMessage("Save an album before adding media.");
      return;
    }
    if (!asset.id && !file) {
    ...
    const { error } = await supabase.rpc("save_media_asset", {
      p_album_id: albumId,
      p_alt_text: asset.altText,
      p_asset_id: asset.id,
      p_caption: asset.caption,
      p_captured_at: asset.capturedAt
        ? new Date(asset.capturedAt).toISOString()
        : null,
      p_credit: asset.credit,
      p_height: asset.height,
      p_is_featured: asset.isFeatured,
      p_mime_type: asset.mimeType,
      p_sort_order: Number(asset.sortOrder) || 0,
  ...
  return (
    <section
      className="admin-section event-gallery-manager"
      id="gallery"
      aria-labelledby="gallery-title"
    >
      <div className="admin-section-heading">
        <div>
          <p className="eyebrow">Secure media</p>
          <h2 id="gallery-title">Event gallery</h2>
          <p>
            Draft images remain private. Publishing issues short-lived delivery
            URLs without making the Storage bucket public.
          </p>
        </div>
      ...
      <div className="gallery-admin-toolbar">
        <label>
          ...
          <select
            value={albumId}
            onChange={(event) => chooseAlbum(event.target.value)}
          >
            <option value="">New album</option>
            {eventAlbums.map((item) => (
              <option key={item.id} value={item.id}>
                {item.title}
              </option>
            ))}
      ...
      <div className="gallery-admin-grid">
        ...
        >
          ...
          <div className="form-grid">
            ...
            <label className="form-wide">
              ...
              <textarea
                rows={4}
                value={album.introduction}
                onChange={(e) =>
                  setAlbum({ ...album, introduction: e.target.value })
                }
              />
            </label>
            <label>
              ...
              <select
                ...
                onChange={(e) =>
                  setAlbum({
                    ...album,
                    status: e.target.value as AdminGalleryAlbum["status"],
                  })
                }
              >
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setAlbum({ ...album, sortOrder: e.target.value })
                }
              />
            </label>
          </div>
        ...
        <div className="gallery-assets-panel">
          <div className="gallery-assets-list">
            <label className="gallery-upload-control">
              <input
                type="file"
              ...
              <small>JPG, PNG or WebP · 10 MB maximum</small>
            </label>
            {assets.map((item) => (
              <button
                type="button"
                ...
                onClick={() => {
                  setAsset(assetForm(item));
                  setFile(null);
                }}
              >
                <span className="gallery-thumb">
                  {item.signed_url ? (
                    <img src={item.signed_url} alt="" />
                  ) : null}
                </span>
                <span>
                  <strong>{item.caption || item.alt_text}</strong>
                  <small>
                    {item.status}
          ...
          >
            ...
            {asset.id || file ? (
              <div className="form-grid">
                <label className="form-wide">
                  ...
                  <input
                    value={asset.altText}
                    onChange={(e) =>
                      setAsset({ ...asset, altText: e.target.value })
                    }
                    required
                  />
                </label>
                <label className="form-wide">
                  ...
                  <textarea
                    ...
                    value={asset.caption}
                    onChange={(e) =>
                      setAsset({ ...asset, caption: e.target.value })
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    value={asset.credit}
                    onChange={(e) =>
                      setAsset({ ...asset, credit: e.target.value })
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    type="datetime-local"
                    value={asset.capturedAt}
                    onChange={(e) =>
                      setAsset({ ...asset, capturedAt: e.target.value })
                    }
                  />
                </label>
                <label>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setAsset({
                        ...asset,
                        status: e.target.value as AdminMediaAsset["status"],
                      })
                    }
                  >
                    <option value="draft">Draft</option>
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAsset({ ...asset, sortOrder: e.target.value })
                    }
                  />
                </label>
                <label className="publish-control">
                  <input
                    ...
                    onChange={(e) =>
                      setAsset({ ...asset, isFeatured: e.target.checked })
                    }
                  />{" "}
                  Featured image
                </label>
              ...
              <div className="admin-empty">
                <strong>Select or upload an image</strong>
                <p>Metadata and publication controls will appear here.</p>
              </div>
            )}
            {asset.id || file ? (
              <button className="button button-primary" disabled={busy}>
