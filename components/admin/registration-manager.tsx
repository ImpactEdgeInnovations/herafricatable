...
export type AdminTicket = {
  ...
  sort_order: number;
  status: "draft" | "on_sale" | "paused" | "sold_out" | "archived";
};
...
const blank = {
  ...
  salesStart: "",
  salesEnd: "",
  status: "draft" as AdminTicket["status"],
  sortOrder: "0",
};
...
export function RegistrationManager({
  ...
  const tickets = initialTickets.filter((x) => x.event_id === eventId);
  ...
  function fail(error: { message: string } | null) {
    ...
    setBusy(false);
    setMessage(adminErrorMessage(error, "complete this registration action"));
    return true;
  }
  async function save(e: FormEvent) {
    ...
    const { error } = await supabase.rpc("save_ticket_type", {
      ...
      p_event_id: eventId,
      p_inventory: form.inventory ? Number(form.inventory) : null,
      p_name: form.name,
      p_price_minor: Math.round(Number(form.price) * 100),
      p_sales_end: form.salesEnd ? new Date(form.salesEnd).toISOString() : null,
      p_sales_start: form.salesStart
  ...
  async function review(orderId: string, action: "approve" | "reject") {
    ...
    setBusy(true);
    const { error } = await supabase.rpc("review_manual_registration", {
      p_action: action,
      p_order_id: orderId,
  ...
  return (
    <>
      <section
        className="admin-section registration-manager"
        id="registrations"
      >
        ...
        <div className="registration-admin-grid">
          <aside className="content-record-list">
            <div>
            ...
            {tickets.map((t) => (
              <button
                key={t.id}
                type="button"
                className={form.id === t.id ? "selected" : ""}
                onClick={() =>
                  setForm({
                    id: t.id,
                    name: t.name,
                    description: t.description ?? "",
                    price: (t.price_minor / 100).toFixed(2),
                    currency: t.currency,
                    inventory: t.inventory_quantity?.toString() ?? "",
                    salesStart: local(t.sales_start_at),
                    salesEnd: local(t.sales_end_at),
                    status: t.status,
              ...
              >
                ...
                <small>
                  {t.currency}{" "}
                  {(t.price_minor / 100).toLocaleString("en-KE", {
          ...
          <form
            ...
            onSubmit={save}
            aria-describedby="ticket-form-guide"
          >
            <p className="admin-form-guide" id="ticket-form-guide">
              Ticket inventory is optional. Sales dates use Nairobi time;
              changing a live ticket can affect registrations already in
              progress.
            </p>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      status: e.target.value as AdminTicket["status"],
                    })
                  }
                >
        ...
        <div className="registration-review">
          <p className="eyebrow">Registration queue</p>
          {registrations.length ? (
                  ...
                  {registrations.map((r) => (
                    <tr key={r.order_id}>
                      ...
                      <td>
                        {r.order_reference}
                        <small>
                          {r.submitted_reference || "No manual reference"}
                        </small>
                      </td>
                      <td>
                        {r.ticket_name} × {r.quantity}
                      </td>
                      <td>
                        {r.currency}{" "}
                        {(r.total_minor / 100).toLocaleString("en-KE", {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      <td>
                      ...
                      <td>
                        {r.status === "pending_review" ? (
                          <div className="member-actions">
                            <button
                              disabled={busy}
                              onClick={() => void review(r.order_id, "approve")}
                            >
                              Approve
                            </button>
                            <button
                              ...
                              disabled={busy}
                              onClick={() => void review(r.order_id, "reject")}
                            >
                              Reject
                            </button>
                          </div>
            ...
            <div className="admin-empty">
              ...
              <p>
                Member requests will appear here with payment mode and review
                evidence.
              </p>
            </div>
          )}
        ...
        {initialPayments.length ? (
          <div className="payment-reconciliation">
            ...
            {initialPayments
              ...
              .map((p) => (
                <article key={p.provider_reference}>
                  <div>
                    <strong>{p.provider_reference}</strong>
                    <span>
                      {
                        registrations.find((r) => r.order_id === p.order_id)
                          ?.order_reference
                      }{" "}
                      · {p.provider}
                    </span>
                  ...
                  <div>
                    <strong>
                      {p.currency}{" "}
                      {(p.amount_minor / 100).toLocaleString("en-KE", {
                        minimumFractionDigits: 2,
                      })}
                    </strong>
                    <span className={`member-status status-${p.status}`}>
                      {p.status}
                    </span>
                  </div>
                </article>
        ...
        {initialRefunds.filter((r) =>
          registrations.some((x) => x.order_id === r.order_id),
        ).length ? (
          <div className="refund-queue">
            ...
            {initialRefunds
              ...
              .map((r) => (
                <article key={r.refund_id}>
                  <div>
                    <strong>{r.display_name || r.email}</strong>
                    <span>
                      {r.order_reference} · {r.currency}{" "}
                      {(r.total_minor / 100).toLocaleString("en-KE", {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                    ...
                    {r.status === "requested" ? (
                      <div className="member-actions">
                        <button
                          disabled={busy}
                          onClick={() =>
                            void reviewRefund(r.refund_id, "approve")
                          }
                        >
                          Approve
                        </button>
                        <button
                          ...
                          disabled={busy}
                          onClick={() =>
                            void reviewRefund(r.refund_id, "reject")
                          }
                        >
                          Reject
                        </button>
