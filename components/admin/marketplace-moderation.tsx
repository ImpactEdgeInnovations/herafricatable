  ...
  async function review(
    ...
    if (action !== "start_review") {
      const result = await ask({
        title:
          action === "hide"
            ? "Hide this marketplace post?"
            : "Dismiss this marketplace report?",
        description:
          action === "hide"
            ? "The post will stop appearing to members while its captured evidence and audit history are preserved."
        ...
        fields: [
          {
            ...
            minLength: 5,
            maxLength: 1000,
            help: "Use at least 5 characters. Keep the decision specific to the reported evidence.",
          },
        ],
      });
      if (!result) return;
    ...
    setBusy(id);
    const { error } = await supabase.rpc("review_marketplace_report", {
      p_action: action,
      p_outcome: outcome,
      p_report_id: id,
    });
    ...
    setMessage(
      error
        ? adminErrorMessage(
            error,
            "record this marketplace moderation decision",
          )
      ...
      >
        <div className="admin-section-heading">
          <div>
            ...
            <h2>Asks &amp; Offers reports</h2>
            <p>
              Review only the captured post snapshot submitted with a report.
              Hiding removes the post while preserving evidence and audit
              history.
            </p>
          </div>
        ...
        {reports.length ? (
            ...
            {reports.map((report) => (
              <article key={report.report_id}>
                ...
                <div>
                  ...
                  <small>Reported by {report.reporter_email}</small>
                </div>
                {["open", "reviewing"].includes(report.status) ? (
          ...
          <div className="admin-empty">
            <strong>No marketplace reports</strong>
            <p>
              Reported Asks and Offers will appear here with their captured
              evidence.
            </p>
          </div>
        )}
