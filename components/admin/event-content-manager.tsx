...
export function EventContentManager({
  ...
  const [sessionForm, setSessionForm] = useState(() =>
    selectedEvent ? defaultSession(selectedEvent) : null,
  );
  ...
  useEffect(() => {
    ...
    let current = true;
    void supabase
      .rpc("list_event_staff", { p_event_id: eventId })
      .then(({ data, error }) => {
        if (!current) return;
  ...
  function speakerFor(sessionId: string) {
    return (
      speakerLinks.find((link) => link.session_id === sessionId)
        ?.event_speakers ?? null
    );
  }
  ...
  function handleError(error: { message: string } | null) {
    ...
    setMessage(adminErrorMessage(error, "save this event content change"));
    return true;
  }
  ...
  async function changeStaff(email: string, action: "assign" | "remove") {
    ...
    setBusy(true);
    setMessage("");
    const { error } = await supabase.rpc("manage_event_staff", {
      p_action: action,
      p_event_id: eventId,
      p_staff_email: email,
    });
    if (handleError(error)) return;
    const { data } = await supabase.rpc("list_event_staff", {
      p_event_id: eventId,
    });
    ...
    setMessage(
      action === "assign"
        ? "Event staff access assigned and audit logged."
        : "Event staff access removed and audit logged.",
    );
  }

  if (!events.length || !selectedEvent || !sessionForm) {
    return (
      <section className="admin-section" id="event-content">
  ...
  return (
    <section
      className="admin-section event-content-manager"
      id="event-content"
      aria-labelledby="event-content-title"
    >
      <div className="admin-section-heading">
        <div>
          <p className="eyebrow">Event content CMS</p>
          <h2 id="event-content-title">Programme and publishing</h2>
          <p>
            Prepare content as drafts, publish deliberately, and keep staff
            access limited to an assigned event.
          </p>
        </div>
      ...
      <div
        ...
        role="tablist"
        aria-label="Event content sections"
      >
        {(["programme", "announcements", "sponsors"] as Panel[]).map((item) => (
          <button
        ...
        <div className="content-workspace">
          <div className="content-record-list">
            <div>
              ...
              <button
                type="button"
                onClick={() => setSessionForm(defaultSession(selectedEvent))}
              >
                New session
              </button>
            </div>
            {sessions.map((item) => {
              const speaker = speakerFor(item.id);
              return (
                <button
                  type="button"
                  key={item.id}
                  className={sessionForm.id === item.id ? "selected" : ""}
                  onClick={() =>
                    setSessionForm({
                      id: item.id,
                      title: item.title,
          ...
          >
            ...
            <div className="form-grid">
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSessionForm({
                      ...sessionForm,
                      status: e.target.value as AdminSession["status"],
                    })
                  }
                >
              ...
              <label>
                Day label
                <input
                  value={sessionForm.dayLabel}
                  onChange={(e) =>
                    setSessionForm({ ...sessionForm, dayLabel: e.target.value })
                  }
                  placeholder="Generated from session date"
                />
              </label>
              <label className="form-wide">
                Description
              ...
              <label>
                Speaker title
                <input
                  value={sessionForm.speakerJobTitle}
                  onChange={(e) =>
                    setSessionForm({
                      ...sessionForm,
                      speakerJobTitle: e.target.value,
                    })
                  }
                />
              </label>
              <label className="form-wide">
                ...
                <input
                  value={sessionForm.speakerCompany}
                  onChange={(e) =>
                    setSessionForm({
                      ...sessionForm,
                      speakerCompany: e.target.value,
                    })
                  }
                />
              </label>
      ...
      {panel === "announcements" ? (
        <div className="content-workspace">
          <div className="content-record-list">
            <div>
              <strong>{announcements.length} announcements</strong>
              <button
                type="button"
                onClick={() => setAnnouncementForm(blankAnnouncement)}
              >
                New update
              </button>
            </div>
            {announcements.map((item) => (
              <button
                type="button"
                key={item.id}
                className={announcementForm.id === item.id ? "selected" : ""}
                onClick={() =>
                  setAnnouncementForm({
                    id: item.id,
                    title: item.title,
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAnnouncementForm({
                      ...announcementForm,
                      status: e.target.value as AdminAnnouncement["status"],
                    })
                  }
                >
                  <option value="draft">Draft</option>
        ...
        <div className="content-workspace">
          <div className="content-record-list">
            <div>
              ...
              <button
                type="button"
                onClick={() => setSponsorForm(blankSponsor)}
              >
                New partner
              </button>
            </div>
            {sponsors.map((item) => (
              <button
                type="button"
                key={item.id}
                className={sponsorForm.id === item.id ? "selected" : ""}
                onClick={() =>
                  setSponsorForm({
                    id: item.id,
                    name: item.name,
            ...
            <div className="form-grid">
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSponsorForm({ ...sponsorForm, tier: e.target.value })
                  }
              ...
              <label className="publish-control">
                <input
                  type="checkbox"
                  checked={sponsorForm.isPublished}
                  onChange={(e) =>
                    setSponsorForm({
                      ...sponsorForm,
      ...
      {panel === "staff" && isSuperAdmin ? (
        <div className="staff-access-panel">
          ...
          >
            ...
            <button className="button button-primary" disabled={busy}>
              {busy ? "Assigning…" : "Assign to this event"}
            </button>
          </form>
          <div className="staff-access-list">
            {staff.length ? (
              staff.map((item) => (
                <article key={item.user_id}>
                  ...
                  <button
                    type="button"
                    disabled={busy}
                    onClick={() => void changeStaff(item.email, "remove")}
                  >
                    Remove access
                  </button>
                </article>
              ...
              <div className="admin-empty">
                <strong>No event staff assigned</strong>
                <p>
                  Assigned staff can manage this event only. They cannot create
                  events or access member administration.
                </p>
              </div>
            )}
          </div>
