...
export async function POST(request:Request){
 try{
  ...
  const body=await request.json() as {attendeeNote?:string;communityOfferId?:string;courseId?:string;eventId?:string;membershipPlanId?:string;quantity?:number;ticketTypeId?:string};
  if(!body.communityOfferId&&!body.courseId&&!body.membershipPlanId&&(!body.eventId||!body.ticketTypeId))return NextResponse.json({error:"A community offer, membership, course or event ticket is required"},{status:400});
  const {data:orderId,error:createError}=body.communityOfferId?await supabase.rpc("create_community_order",{p_offer_id:body.communityOfferId,p_manual_note:"",p_manual_reference:""}):body.membershipPlanId?await supabase.rpc("create_membership_order",{p_plan_id:body.membershipPlanId,p_manual_note:"",p_manual_reference:""}):body.courseId?await supabase.rpc("create_course_order",{p_course_id:body.courseId,p_manual_note:"",p_manual_reference:""}):await supabase.rpc("create_event_registration",{p_attendee_note:body.attendeeNote??"",p_event_id:body.eventId,p_manual_note:"",p_manual_reference:"",p_quantity:Number(body.quantity)||1,p_ticket_type_id:body.ticketTypeId});
  if(createError||!orderId)return NextResponse.json({error:createError?.message??"Order creation failed"},{status:400});
  const {data:order,error:orderError}=await supabase.from("orders").select("id,reference,total_minor,currency,event_id,order_type,order_items(course_id,membership_plan_id,community_offer_id)").eq("id",orderId).single();
  if(orderError||!order)return NextResponse.json({error:"Order could not be loaded"},{status:500});
  try{
   const item=(order.order_items as unknown as {community_offer_id:string|null;course_id:string|null;membership_plan_id:string|null}[]|null)?.[0];
   const initialized=await initializePaystackTransaction({email:user.email,amount:order.total_minor,currency:order.currency,reference:order.reference,callbackUrl:`${siteUrl}/api/payments/paystack/callback`,metadata:{order_id:order.id,order_type:order.order_type,event_id:order.event_id??"",community_offer_id:item?.community_offer_id??"",course_id:item?.course_id??"",membership_plan_id:item?.membership_plan_id??""}});
