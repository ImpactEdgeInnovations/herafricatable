...
export async function POST(request:Request){
 try{
  ...
  const body=await request.json() as {attendeeNote?:string;eventId?:string;quantity?:number;ticketTypeId?:string};
  if(!body.eventId||!body.ticketTypeId)return NextResponse.json({error:"Event and ticket are required"},{status:400});
  const {data:orderId,error:createError}=await supabase.rpc("create_event_registration",{p_attendee_note:body.attendeeNote??"",p_event_id:body.eventId,p_manual_note:"",p_manual_reference:"",p_quantity:Number(body.quantity)||1,p_ticket_type_id:body.ticketTypeId});
  if(createError||!orderId)return NextResponse.json({error:createError?.message??"Order creation failed"},{status:400});
  const {data:order,error:orderError}=await supabase.from("orders").select("id,reference,total_minor,currency,event_id").eq("id",orderId).single();
  if(orderError||!order)return NextResponse.json({error:"Order could not be loaded"},{status:500});
  try{
   const initialized=await initializePaystackTransaction({email:user.email,amount:order.total_minor,currency:order.currency,reference:order.reference,callbackUrl:`${siteUrl}/api/payments/paystack/callback`,metadata:{order_id:order.id,event_id:order.event_id}});
   ...
   return NextResponse.json({error:"Secure checkout could not be initialized. Please retry or use the event's manual option if enabled."},{status:502});
  }
