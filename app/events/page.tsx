  ...
  return (
    <main className="events-page">
      ...
      <section className="public-event-list" aria-label="Published events">
        ...
        )) : <div className="events-empty"><span className="events-empty-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="4" y="5" width="16" height="15" rx="2"/><path d="M8 3v4M16 3v4M4 10h16"/></svg></span><div><p className="eyebrow">Nothing to book yet</p><strong>The next table is being prepared.</strong><p>{isActiveMember ? "We will alert you as soon as the date, venue, and registration window are ready." : "Published event details will appear here. Join the founding network to hear first."}</p><div className="events-empty-actions"><Link className="button button-primary" href={isActiveMember ? "/home" : "/sign-in"}>{isActiveMember ? "Return home" : "Request membership"}</Link>{isActiveMember ? <Link className="button button-outline" href="/network">Meet members</Link> : null}</div></div></div>}
      </section>
    </main>
  );
}
