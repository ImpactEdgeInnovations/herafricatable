...
export default async function OpportunitiesPage({ searchParams }: { searchParams: Promise<{ category?: string; compose?: string; mine?: string; q?: string; type?: string }> }) {
  ...
  return <main className="opportunity-page"><MemberHeader label="Asks & Offers"/><section className="opportunity-hero"><div><p className="eyebrow">Help moves through the table</p><h1>Ask for what you need.<br />Offer what you know.</h1><p>Share one clear request, useful skill or opportunity with trusted members. Replies stay private between you and the person who responds.</p></div><Link className="button button-primary" href="/opportunities?compose=1#create-opportunity">Share an Ask or Offer</Link></section>{result.error ? <section className="admin-empty opportunity-error"><strong>Asks &amp; Offers are temporarily unavailable</strong><p>Please try again or contact support if the problem continues.</p><div className="journey-state-actions"><Link className="button button-primary" href="/opportunities">Try again</Link><Link className="button button-outline" href="/support">Contact support</Link></div></section> : <OpportunityMarketplace currentUserId={user.id} initialComposerOpen={filters.compose === "1"} initialPosts={posts} initialResponses={responses} />}</main>;
}
