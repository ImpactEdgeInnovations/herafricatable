...
export default async function MemberHomePage() {
  ...
  const { data: orderRows } = await supabase.from("orders").select("id,reference,status,processing_mode,currency,total_minor,created_at,order_type,events(title,slug),order_items(ticket_types(name),courses(title,slug))").eq("user_id",user.id).order("created_at",{ascending:false});
  const orderIds=(orderRows??[]).map((order)=>order.id);
  const { data: refunds }=orderIds.length?await supabase.from("refund_requests").select("order_id").in("order_id",orderIds):{data:[]};
  const orders:MemberOrder[]=(orderRows??[]).map((order)=>{const item=(order.order_items as unknown as {ticket_types:{name:string}|null;courses:{slug:string;title:string}|null}[])[0];return{course:item?.courses??null,created_at:order.created_at,currency:order.currency,event:order.events as unknown as {slug:string;title:string}|null,id:order.id,order_type:order.order_type,processing_mode:order.processing_mode,reference:order.reference,status:order.status,ticket_name:item?.ticket_types?.name??(item?.courses?"Course access":"Event ticket"),total_minor:order.total_minor}});
  ...
  return (
    <main className="member-home-page">
      <header className="member-home-header"><Link className="brand" href="/"><span className="brand-mark">H</span><span>Her Africa Table<small>Member house</small></span></Link><nav>{accessStatus==="active"?<><Link href="/network">Member network</Link><Link href="/opportunities">Asks &amp; Offers</Link>{communityFlagResult.data?.enabled?<Link href="/communities">Communities</Link>:null}{learningFlagResult.data?.enabled?<Link href="/learning">Learning</Link>:null}{referralFlagResult.data?.enabled?<Link href="/referrals">Invite</Link>:null}<Link href="/messages">Messages</Link></>:null}<Link href="/events">Upcoming tables</Link><Link href="/notifications">Notifications</Link><Link href="/support">Support</Link><Link href="/settings">Settings</Link></nav></header>
